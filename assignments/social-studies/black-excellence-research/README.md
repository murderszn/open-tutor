# 🌟 Black Excellence — Research Project

Subject: Social Studies (Arts & Culture)

## Overview
Research influential Black leaders, business people, government officials, and entertainers and their impact on culture. Your goal is to learn who they are, what they contributed, and why their work matters.

## Tasks
- Pick 3 figures from completely different walks of life (e.g., business, politics, science, entertainment). Try to include at least one from earlier history and one modern figure.
- For each person, include:
  - 5 bullet facts (signature works, awards, time period, notable firsts/barriers overcome, cultural impact)
  - 1–2 representative images or media references (title + year is fine)
  - 1 source (book/article/biography/interview). Cite in a simple bullet with the title and where you found it.
- Add a short reflection (5–7 sentences) comparing the three figures. What patterns do you notice? How did their eras shape their opportunities?

## Deliverables (pick one)
- 1–2 page document (Google Doc or Markdown) with sections for each person
- OR a 3–6 slide mini‑deck (one or two slides per person) with a final comparison slide

Save your work in your student folder:
- Caleb: `students/caleb/grade-7/social-studies/black-excellence/`
- Elijah: `students/elijah/grade-4/social-studies/black-excellence/`

## Suggested Starters & Resources
We have compiled an extensive list of incredible historic and modern figures to research. 
👉 **Check out the [Black Excellence Figures Guide](../../../resources/black_excellence_figures.md) to pick your 3 people.**

Alternatively, look into:
- Music: Louis Armstrong, Aretha Franklin, Stevie Wonder, Whitney Houston
- Comedy: Richard Pryor, Eddie Murphy, Whoopi Goldberg, Dave Chappelle

Use reputable sources (books, documentaries, museum archives, biographies). Avoid unsourced trivia.

## Grading Checklist
- [ ] 3 figures from different fields or eras
- [ ] 5 accurate facts per person with at least 1 cited source
- [ ] Reflection compares patterns and differences
- [ ] Clear organization and readable formatting

—

Due date (for this week planning): March 27, 2026

