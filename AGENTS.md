...
- `assets/`, `public/` — shared visuals and static hosting.
...
- Preview HTML: `python3 -m http.server 8000` and open `http://localhost:8000` (or `open path/to/index.html` on macOS).
  - Examples: `assignments/personal-finance/credit-scores-101/index.html`, `assignments/stem/biology/resources/water-cycle/index.html`.
- Fast search: `rg -n "keyword"` and list files with `rg --files` (use `--pcre2` for lookarounds).
...
- Link sanity: search for relative links after moves/renames: `rg -n --pcre2 "\]\((?!http|#).+\)"`.
...
- When adding a new assignment:
  ...
  - Update indexes: `assignments/README.md`, student STEM `README.md`, root `README.md` if featured.
  - Add a Resource Pack (see below) to the spec and student stubs.
...
- Quick‑open helpers: Google/YouTube search URLs.
